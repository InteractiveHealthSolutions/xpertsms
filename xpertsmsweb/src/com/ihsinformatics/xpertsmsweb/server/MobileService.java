...
public class MobileService extends HttpServlet {
	...
	if (request.getParameter("chwid") != null) {
	    try {
		uCheck = ssl.exists("Users", "where PID = '"
			+ request.getParameter("chwid").toUpperCase() + "'");
	    } catch (Exception e) {
		e.printStackTrace();
	    }
		...
		out.println(XmlUtil
			.createErrorXml("Aap ne ghalat CHW ID ya Monitor ID darj kia hai. Sahih ID enter karain aur dobara koshish karain"));
	    }
	}
	...
	if (request.getParameter("mid") != null) {
	    try {
		uCheck = ssl.exists("Users", "where PID = '"
			+ request.getParameter("mid").toUpperCase() + "'");
	    } catch (Exception e) {
		e.printStackTrace();
	    }
	...
	if (request.getParameter("gpid") != null) {
	    try {
		uCheck = ssl.exists("Users", "where PID = '"
			+ request.getParameter("gpid").toUpperCase() + "'");
	    } catch (Exception e) {
		e.printStackTrace();
	    }
	    ...
	    if (uCheck.booleanValue() != true) {
		out.println(XmlUtil
			.createErrorXml("Aap ne ghalat GP ID darj kia hai. Sahih ID enter karain aur dobara koshish karain"));
	    }
	}
	...
	if (gxpO == null) {
	    ...
	    GeneXpertResultsUnlinked gxpU = createGeneXpertResultsUnlinked(mtb,
		    rif, resultDate, instrumentSerial, moduleId, cartridgeId,
		    reagentLotId, pcId, probeResultA, probeResultB,
		...
		ssl.saveGeneXpertResultsUnlinked(gxpU);// ,
						       // gxp.getIsPositive(),operatorId,pcId,instrumentSerial,moduleId,cartridgeId,reagentLotId);
	    } catch (Exception e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	 ...
	 * gxp.setRemarks(gxpO.getRemarks()); gxp.setSputumTestId("" +
	 * gxpO.getSputumTestId());
	 */
	...
	/*
	 * String operatorId = request.getParameter("operatorid");//=" +
	 * operatorId;// e.g Karachi Xray String pcId =
	 * request.getParameter("pcid"); String instrumentSerial =
	 * request.getParameter("instserial"); String moduleId =
	 * request.getParameter("moduleid"); String cartridgeId =
	 * request.getParameter("cartrigeid"); String reagentLotId =
	 * request.getParameter("reagentlotid");
	 */
	try {
	    // ssl.updateGeneXpertResultsAuto(gxp,
	    // gxp.getIsPositive(),operatorId,pcId,instrumentSerial,moduleId,cartridgeId,reagentLotId);
	    ssl.saveGeneXpertResults(gxpO);// ,
					   // gxp.getIsPositive(),operatorId,pcId,instrumentSerial,moduleId,cartridgeId,reagentLotId);
	} catch (Exception e) {
	    // TODO Auto-generated catch block
	    e.printStackTrace();
	    return XmlUtil.createErrorXml("Could not save data for Sample ID "
		    + gxpO.getSputumTestId());
	}

	// }
	/*
	 * postParams += "type=astmresult"; postParams += "&sampleid=" +
	 * sampleId; postParams += "&mtb=" + mtbResult; postParams += "&rif=" +
	 * rifResult;
	 * 
	 * if(isFinal()) postParams += "&final=yes"; if(isPending()) postParams
	 * += "&pending=yes"; if(isError()) postParams += "&error=yes";
	 * if(isFinal()) postParams += "&final=yes";
	 * 
	 * return postParams;
	 */
    ...
    public GeneXpertResultsUnlinked createGeneXpertResultsUnlinked(String mtb,
	    String rif, String resultDate, String instrumentSerial,
