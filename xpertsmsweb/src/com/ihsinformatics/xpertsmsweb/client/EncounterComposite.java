...
public class EncounterComposite extends Composite implements IForm,
    ...
    private Label lblXpertSmsEncounters = new Label("XpertSMS Encounters");
	...
	lblXpertSmsEncounters.setStyleName("title");
	topFlexTable.setWidget(0, 0, lblXpertSmsEncounters);
	...
	encounterStartDateBox.setFormat(new DefaultFormat(DateTimeFormat
	...
	encounterEndDateBox.setFormat(new DefaultFormat(DateTimeFormat
		.getFormat("dd-MM-yyyy HH:mm")));
	rightFlexTable.setWidget(6, 1, encounterEndDateBox);
	rightFlexTable.setWidget(7, 0, lblEncounterEnteredDate);
	encounterEnteredDateBox.setFormat(new DefaultFormat(DateTimeFormat
		.getFormat("dd-MM-yyyy HH:mm")));
	...
	try {
	    ...
	    for (String s : XSMS.getList(ListType.ENCOUNTER_TYPE))
	...
	try {
	    ...
	    current.setDetails(XpertSmsWebClient.get(detailsTextBox));
	} catch (Exception e) {
	    e.printStackTrace();
	}
    }
	...
	try {
	    String selection = XpertSmsWebClient.get(mainListBox);
	    String table;
		...
		table = "Monitor";
	    service.getColumnData(table, table + "ID", "",
	...
	try {
	    service.getColumnData(tableName, "PID1", "PID2='"
		    + XpertSmsWebClient.get(staffListBox) + "'",
		    new AsyncCallback<String[]>() {
			@Override
			public void onSuccess(String[] result) {
	...
	try {
	    service.getColumnData(tableName, "EncounterID", "PID1='"
		    + XpertSmsWebClient.get(patientListBox) + "' AND PID2='"
		    + XpertSmsWebClient.get(staffListBox) + "'",
		    new AsyncCallback<String[]>() {
			@Override
			public void onSuccess(String[] result) {
	...
	try {
	    EncounterId encounterID = new EncounterId(
		    Integer.parseInt(XpertSmsWebClient.get(encounterIdListBox)),
		    XpertSmsWebClient.get(patientIdTextBox), XpertSmsWebClient
			    .get(staffTextBox));
	    service.findEncounter(encounterID, new AsyncCallback<Encounter>() {
		@Override
		public void onSuccess(Encounter result) {
		    current = result;
		    encounterTypeComboBox.setSelectedIndex(XpertSmsWebClient
			    .getIndex(encounterTypeComboBox,
				    current.getEncounterType()));
		    encounterStartDateBox.setValue(current
			    .getDateEncounterStart());
		    encounterEndDateBox.setValue(current.getDateEncounterEnd());
		    encounterEnteredDateBox.setValue(current
			    .getDateEncounterEntered());
		    detailsTextBox.setValue(current.getDetails());
		    try {
			String filter = "EncounterID="
				+ current.getId().getEncounterId() + " AND "
				+ "PID1='" + current.getId().getPid1()
				+ "' AND " + "PID2='"
				+ current.getId().getPid2() + "'";
			String[] columns = { "Element", "Value" };
			service.getTableData("EncounterResults", columns,
				filter, new AsyncCallback<String[][]>() {
				    @Override
				    public void onSuccess(String[][] result) {
	...
	if (XpertSmsWebClient.get(patientIdTextBox).equals("")
		|| XpertSmsWebClient.get(staffTextBox).equals("")) {
    ...
    /**
     * Updates encounter with results. The results with 'delete' check boxes
     * checked get deleted, the others update
     */
    @Override
	...
	if (validate()) {
		...
		service.updateEncounter(current, new AsyncCallback<Boolean>() {
			...
			for (int i = 0; i < resultsGrid.getRowCount(); i++) {
				...
				CheckBox chk = (CheckBox) resultsGrid
					.getWidget(i, 2);
				if (chk.getValue()) {
				    Label lbl = (Label) resultsGrid.getWidget(
					    i, 0);
				    EncounterResultsId id = new EncounterResultsId(
					    current.getId().getEncounterId(),
					    current.getId().getPid1(), current
						    .getId().getPid2(), lbl
						    .getText());
				    try {
					service.deleteEncounterResults(id,
				    ...
				    EncounterResultsId id = new EncounterResultsId(
					    ...
					    current.getId().getPid1(), current
						    ...
						    .getText());
				    try {
					TextBox txt = (TextBox) resultsGrid
						.getWidget(i, 1);
					service.updateEncounterResults(id,
						txt.getValue(),
						new AsyncCallback<Boolean>() {
						    @Override
						    public void onSuccess(
	...
	if (validate()) {
	    try {
		if (Window
			.confirm("This will delete following Encounter with results:\n"
				+ current.toString()
				+ "\n"
				+ CustomMessage
					.getInfoMessage(InfoType.CONFIRM_OPERATION))) {
		    service.deleteEncounterWithResults(current,
			    new AsyncCallback<Boolean>() {
				@Override
				public void onSuccess(Boolean result) {
				    if (result) {
	...
	if (sender == mainListBox) {
	    ...
	    fillPatientList();
	    staffTextBox.setValue(XpertSmsWebClient.get(staffListBox));
	} else if (sender == patientListBox) {
	    fillEncounterList();
	    patientIdTextBox.setValue(XpertSmsWebClient.get(patientListBox));
	    try {
		service.getObject(
			"Person",
			"CONCAT(IFNULL(FirstName, ''), ' ', IFNULL(LastName, '')) AS FullName",
			"PID='" + XpertSmsWebClient.get(patientIdTextBox) + "'",
			new AsyncCallback<String>() {
			    @Override
			    public void onSuccess(String result) {
