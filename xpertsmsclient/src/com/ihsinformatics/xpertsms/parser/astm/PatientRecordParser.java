...
/**
 * Parser for Patient record block of GeneXpert results in ASTM standard
...
public class PatientRecordParser extends BaseParser {
	...
	public void parse() throws InvalidASTMMessageFormatException {
		...
		if (fields.length > ASTMMessageConstants.MAX_NUM_PATIENT_RECORD_FIELDS) {
			throw new InvalidASTMMessageFormatException("P001 - Patient record must have at most "
			        + ASTMMessageConstants.MAX_NUM_PATIENT_RECORD_FIELDS + " fields");
		}
		if (fields.length == 5) {
	...
	public void setPatientId(String field) throws InvalidASTMMessageFormatException {
		if (field.length() < 1 || field.length() > ASTMMessageConstants.MAX_PATIENT_ID_LENGTH) {
			throw new InvalidASTMMessageFormatException("P002 - Patient ID must be between 1 and "
			        + ASTMMessageConstants.MAX_PATIENT_ID_LENGTH + " characters");
		}
