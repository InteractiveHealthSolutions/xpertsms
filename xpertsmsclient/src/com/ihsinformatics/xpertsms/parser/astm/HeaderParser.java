...
public class HeaderParser extends BaseParser {
	...
	public void parse() throws InvalidASTMMessageFormatException {
		...
		if (fields.length != ASTMMessageConstants.NUM_HEADER_FIELDS) {
			throw new InvalidASTMMessageFormatException("H001 - Header record must have "
			        + ASTMMessageConstants.NUM_HEADER_FIELDS + " fields");
		}
		// set delimiters
		// 0 is H
	...
	private void setDelimiters(String field) throws InvalidASTMMessageFormatException {
		...
		XpertASTMResultUploadMessage.REPEAT_DELIMITER = delimiters.charAt(0);
		if (delimiters.length() > 1) {
			XpertASTMResultUploadMessage.COMPONENT_DELIMITER = delimiters.charAt(1);
		}
		if (delimiters.length() == 2) {
			XpertASTMResultUploadMessage.ESCAPE_DELIMITER = delimiters.charAt(2);
		}
	}
	...
	private void setMessageId(String field) throws InvalidASTMMessageFormatException {
		if (field.length() < 1 || field.length() > ASTMMessageConstants.MAX_MESSAGE_ID_LENGTH) {
			throw new InvalidASTMMessageFormatException("H003 - Message ID must be between 1 and "
			        + ASTMMessageConstants.MAX_MESSAGE_ID_LENGTH + " characters");
		}
		record.setMessageId(field);
	}
	
	private void setSystemDetails(String field) throws InvalidASTMMessageFormatException {
		String systemDetails[] = field.split("\\" + XpertASTMResultUploadMessage.COMPONENT_DELIMITER);
		...
		if (systemId.length() < 1 || systemId.length() > ASTMMessageConstants.MAX_SYSTEM_ID_LENGTH) {
			throw new InvalidASTMMessageFormatException("H005 - System ID must be between 1 and "
			        + ASTMMessageConstants.MAX_SYSTEM_ID_LENGTH + " characters");
		}
		...
		if (softwareVersion.length() < 1 || softwareVersion.length() > ASTMMessageConstants.MAX_SOFTWARE_VERSION_LENGTH) {
			throw new InvalidASTMMessageFormatException("H007 - Software Version must be between 1 and "
			        + ASTMMessageConstants.MAX_SOFTWARE_VERSION_LENGTH + " characters");
		}
	...
	private void setReceiverId(String field) throws InvalidASTMMessageFormatException {
		if (field.length() < 1 || field.length() > ASTMMessageConstants.MAX_RECV_ID_LENGTH) {
			throw new InvalidASTMMessageFormatException("H008 - Receiver ID must be between 1 and "
			        + ASTMMessageConstants.MAX_RECV_ID_LENGTH + " characters");
		}
	...
	private void setVersionNumber(String field) throws InvalidASTMMessageFormatException {
		if (!field.equals(ASTMMessageConstants.VERSION_NUMBER)) {
			throw new InvalidASTMMessageFormatException("H010 - Processing ID must be \""
			        + ASTMMessageConstants.VERSION_NUMBER + "\"");
		}
	...
	private void setMessageDateTime(String field) throws InvalidASTMMessageFormatException {
		if (field.length() < 1 || field.length() > ASTMMessageConstants.MAX_DATE_LENGTH) {
			throw new InvalidASTMMessageFormatException("H011 - Date must be " + ASTMMessageConstants.MAX_DATE_LENGTH
			        + " characters");
		}
