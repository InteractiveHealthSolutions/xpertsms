...
public class LabResultsComposite extends Composite implements IForm, ClickHandler, ChangeHandler
{
	...
	private TextBox						MRNoSearchTextBox		= new TextBox();
	...
	public LabResultsComposite()
	{
		...
		leftFlexTable.setWidget(1, 0, MRNoSearchTextBox);
		MRNoSearchTextBox.setVisibleLength(12);
		MRNoSearchTextBox.setMaxLength(12);
	...
	public void refreshList()
	{
		try
		{
			service.getObject("Patient", "PatientID", "MRNo='" + TBRClient.get(MRNoSearchTextBox) + "'", new AsyncCallback<String>()
			{
				@Override
				public void onSuccess(String result)
				{
					...
					try
					{
						service.getColumnData(tableName, "SputumTestID", "PatientID='" + currentPatientID + "' and Remarks not like 'REJECTED%'",
	...
	public void setCurrent()
	{
		...
		current.setSmearResult(TBRClient.get(smearResultsComboBox));
		//current.setIrs(Integer.parseInt(TBRClient.get(IRSTextBox)));
		current.setRemarks(TBRClient.get(remarksTextBox));
	}
	...
	public boolean validate()
	{
		...
		if (TBRClient.get(patientIdTextBox).equals("") || TBRClient.get(sputumTestIdTextBox).equals("") || TBRClient.get(IRSTextBox).equals(""))
		{
	...
	public void updateData()
	{
		if (validate())
		{
			try
			{
				Boolean isTBPositive = !TBRClient.get(smearResultsComboBox).equalsIgnoreCase("NEGATIVE");
				if (isTBPositive)
				{
