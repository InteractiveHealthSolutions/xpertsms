...
public class GeneXpertResultsComposite extends Composite implements IForm, ClickHandler, ChangeHandler
{
	...
	private Label						lblTbReachLab			= new Label("TB REACH Gene Xpert Results");
	private Label						lblPatientId			= new Label("Patient ID:");
	private Label						lblSputumTestId			= new Label("Test ID:");
	//private Label						lblPatientsuspectId		= new Label("MR No.");
	//private Label						lblIrs					= new Label("IRS:");
	...
	public GeneXpertResultsComposite()
	{
		...
		/*
		 * instrumentIdTextBox		= new TextBox();
	private TextBox						moduleIdTextBox			= new TextBox();
	private TextBox						cartidgeIdTextBox		= new TextBox();
	private TextBox						reagentLotIdTextBox		= new TextBox();
	private TextBox						pcIdTextBox	
		...
		mainStackPanel.add(instrumentFlexTable, "Instrument and Materials Data", false);
	...
	public void refreshList()
	{
		try
		{
			/*service.getObject("GeneXpertResults", "TestID", "SputumTestID='" + TBRClient.get(testIdSearchTextBox) + "' AND PatientID='" + TBRClient.get(pidSearchTextBox) + "'", new AsyncCallback<String>()
			{
				@Override
				public void onSuccess(String result)
				{
					...
					try
					{
						service.getColumnData("GeneXpertResults", "TestID", "SputumTestID='" + TBRClient.get(testIdSearchTextBox) + "' AND PatientID='" + TBRClient.get(pidSearchTextBox) + "'"
	...
	public void fillData()
	{
		...
		try
		{
			//service.findGeneXpertResults(TBRClient.get(sputumIdListBox), new AsyncCallback<GeneXpertResults>()
			service.findGeneXpertResultsByTestID(TBRClient.get(sputumIdListBox), new AsyncCallback<GeneXpertResults>()
			{
				@Override
				public void onSuccess(GeneXpertResults result)
				{
					...
					current = result;
					sputumTestID.setText(String.valueOf(current.getSputumTestId()));
					patientIdTextBox.setText(current.getPatientId());
					//irsTextBox.setText(String.valueOf(current.getIrs()));
					geneXpertResultComboBox.setSelectedIndex(TBRClient.getIndex(geneXpertResultComboBox, current.getGeneXpertResult()));
					mtbBurdenComboBox.setSelectedIndex(TBRClient.getIndex(mtbBurdenComboBox, current.getMtbBurden()));
					drugResistanceComboBox.setSelectedIndex(TBRClient.getIndex(drugResistanceComboBox, current.getDrugResistance()));
					errorCodeIntegerBox.setValue(current.getErrorCode());
					remarksTextBox.setText(current.getRemarks());
					
					instrumentIdTextBox.setText(current.getInstrumentSerial());
					moduleIdTextBox.setText(current.getModuleId());
					cartidgeIdTextBox.setText(current.getCartridgeId());
					reagentLotIdTextBox.setText(current.getReagentLotId());
					pcIdTextBox.setText(current.getPcId());
					operatorIdTextBox.setText(current.getOperatorId());
					
					probeResultAListBox.setSelectedIndex(TBRClient.getIndex(probeResultAListBox, current.getProbeResultA()));
					probeResultBListBox.setSelectedIndex(TBRClient.getIndex(probeResultBListBox, current.getProbeResultB()));
					probeResultCListBox.setSelectedIndex(TBRClient.getIndex(probeResultCListBox, current.getProbeResultC()));
					probeResultDListBox.setSelectedIndex(TBRClient.getIndex(probeResultDListBox, current.getProbeResultD()));
					probeResultEListBox.setSelectedIndex(TBRClient.getIndex(probeResultEListBox, current.getProbeResultE()));
					probeResultSPCListBox.setSelectedIndex(TBRClient.getIndex(probeResultSPCListBox, current.getProbeResultSPC()));
					...
					probeCtSPCDoubleBox.setValue(current.getProbeCtSPC());
					
					probeEndptADoubleBox.setValue(current.getProbeEndptA());
					probeEndptBDoubleBox.setValue(current.getProbeEndptB());
					probeEndptCDoubleBox.setValue(current.getProbeEndptC());
					probeEndptDDoubleBox.setValue(current.getProbeEndptD());
					probeEndptEDoubleBox.setValue(current.getProbeEndptE());
					probeEndptSPCDoubleBox.setValue(current.getProbeEndptSPC());
					load(false);
				}
	...
	{
		String result = TBRClient.get(geneXpertResultComboBox);
		...
		{
			current.setDrugResistance(TBRClient.get(drugResistanceComboBox));
			current.setMtbBurden(TBRClient.get(mtbBurdenComboBox));
		}
		...
		//Materials
		current.setInstrumentSerial(TBRClient.get(instrumentIdTextBox).toUpperCase());
		current.setModuleId(TBRClient.get(moduleIdTextBox).toUpperCase());
		current.setReagentLotId(TBRClient.get(reagentLotIdTextBox).toUpperCase());
		current.setCartridgeId(TBRClient.get(cartidgeIdTextBox).toUpperCase());
		current.setPcId(TBRClient.get(pcIdTextBox).toUpperCase());
		current.setOperatorId(TBRClient.get(operatorIdTextBox).toUpperCase());
		
		//Probes
		if(probeResultAListBox.getSelectedIndex()!=-1)
			current.setProbeResultA(TBRClient.get(probeResultAListBox).toUpperCase());
		if(probeResultBListBox.getSelectedIndex()!=-1)
			current.setProbeResultB(TBRClient.get(probeResultBListBox).toUpperCase());
		if(probeResultCListBox.getSelectedIndex()!=-1)
			current.setProbeResultC(TBRClient.get(probeResultCListBox).toUpperCase());
		if(probeResultDListBox.getSelectedIndex()!=-1)
			current.setProbeResultD(TBRClient.get(probeResultDListBox).toUpperCase());
		if(probeResultEListBox.getSelectedIndex()!=-1)
			current.setProbeResultE(TBRClient.get(probeResultEListBox).toUpperCase());
		if(probeResultSPCListBox.getSelectedIndex()!=-1)
			current.setProbeResultSPC(TBRClient.get(probeResultSPCListBox).toUpperCase());
		
		if(TBRClient.get(probeCtADoubleBox).length()!=0)
			current.setProbeCtA(Double.parseDouble(TBRClient.get(probeCtADoubleBox)));
		if(TBRClient.get(probeCtBDoubleBox).length()!=0)
			current.setProbeCtB(Double.parseDouble(TBRClient.get(probeCtBDoubleBox)));
		if(TBRClient.get(probeCtCDoubleBox).length()!=0)
			current.setProbeCtC(Double.parseDouble(TBRClient.get(probeCtCDoubleBox)));
		if(TBRClient.get(probeCtDDoubleBox).length()!=0)
			current.setProbeCtD(Double.parseDouble(TBRClient.get(probeCtDDoubleBox)));
		if(TBRClient.get(probeCtEDoubleBox).length()!=0)
			current.setProbeCtE(Double.parseDouble(TBRClient.get(probeCtEDoubleBox)));
		if(TBRClient.get(probeCtSPCDoubleBox).length()!=0)
			current.setProbeCtSPC(Double.parseDouble(TBRClient.get(probeCtSPCDoubleBox)));
		
		if(TBRClient.get(probeEndptADoubleBox).length()!=0)
			current.setProbeEndptA(Double.parseDouble(TBRClient.get(probeEndptADoubleBox)));
		if(TBRClient.get(probeEndptBDoubleBox).length()!=0)
			current.setProbeEndptB(Double.parseDouble(TBRClient.get(probeEndptBDoubleBox)));
		if(TBRClient.get(probeEndptCDoubleBox).length()!=0)
			current.setProbeEndptC(Double.parseDouble(TBRClient.get(probeEndptCDoubleBox)));
		if(TBRClient.get(probeEndptDDoubleBox).length()!=0)
			current.setProbeEndptD(Double.parseDouble(TBRClient.get(probeEndptDDoubleBox)));
		if(TBRClient.get(probeEndptEDoubleBox).length()!=0)
			current.setProbeEndptE(Double.parseDouble(TBRClient.get(probeEndptEDoubleBox)));
		if(TBRClient.get(probeEndptSPCDoubleBox).length()!=0)
			current.setProbeEndptSPC(Double.parseDouble(TBRClient.get(probeEndptSPCDoubleBox)));
	}
	...
	public boolean validate()
	{
		...
		if (errorCodeIntegerBox.isEnabled() && TBRClient.get(errorCodeIntegerBox).equals(""))
		{
	...
	public void updateData()
	{
		if (validate())
		{
			try
			{
				Boolean isTBPositive = (TBRClient.get(geneXpertResultComboBox).equals("DETECTED"));
				if (isTBPositive)
				{
				...
				setCurrent();
				service.updateGeneXpertResults(current, isTBPositive, new AsyncCallback<Boolean>()
				{
					@Override
					public void onSuccess(Boolean result)
					{
	...
	public void onChange(ChangeEvent event)
	{
		...
		{
			mtbBurdenComboBox.setEnabled(TBRClient.get(geneXpertResultComboBox).equals("MTB DETECTED"));
			drugResistanceComboBox.setEnabled(TBRClient.get(geneXpertResultComboBox).equals("MTB DETECTED"));
			errorCodeIntegerBox.setEnabled(TBRClient.get(geneXpertResultComboBox).equals("ERROR"));
			load(false);
		}
	}
}
