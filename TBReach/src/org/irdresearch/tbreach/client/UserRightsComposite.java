...
import org.irdresearch.tbreach.shared.model.UserRightsId;
...
public class UserRightsComposite extends Composite implements IForm, ClickHandler, ChangeHandler
{
	...
	private Label						lblTbReachUser	= new Label("TB REACH User Rights");
	...
	private void refreshLists()
	{
		...
		for (String s : TBR.getList(ListType.MENU_NAME))
	...
	public void saveData()
	{
		if (validate())
		{
			...
			try
			{
				service.exists(tableName, "ColumnName='" + "'", new AsyncCallback<Boolean>()
				{
					@Override
	...
	public void fillData()
	{
		try
		{
			service.getRowRecord("UserRights", new String[] { "SearchAccess", "InsertAccess", "UpdateAccess", "DeleteAccess", "PrintAccess" },
					"Role='" + TBRClient.get(rolesListBox) + "' and MenuName='" + TBRClient.get(menuComboBox) + "'", new AsyncCallback<String[]>()
					{
						@Override
						public void onSuccess(String[] result)
						{
							...
							current = new UserRights(new UserRightsId(TBRClient.get(rolesListBox), TBRClient.get(menuComboBox)), rights[0],
									rights[1], rights[2], rights[3], rights[4]);
						...
						public void onFailure(Throwable caught)
						{
							...
							try
							{
								UserRights userRights = new UserRights(new UserRightsId(TBRClient.get(rolesListBox), TBRClient.get(menuComboBox)), false, false, false, false, false);
