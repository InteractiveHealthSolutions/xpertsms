...
public class MessageSettingsComposite extends Composite implements IForm, ClickHandler, ChangeHandler, ValueChangeHandler<Boolean>
{
			...
			service.findMessageSettings(new AsyncCallback<MessageSettings>()
			{
				...
				public void onSuccess(MessageSettings result)
				{
					...
					try
					{
						...
						String locationHealthFacilityType =  "HEALTH FACILITY";
						service.findLocationsByType (locationDistrictType, new AsyncCallback<Location[]>(){
							...
							public void onSuccess(Location[] result)
							{
								...
								for (Location s : result)
									locationDistrictComboBox.addItem (s.getLocationName (), s.getLocationId ());
								load (false);
								
								service.findFacilitiesByDistrictId (TBRClient.get (locationDistrictComboBox), new AsyncCallback<Location[]> ()
										{
											@Override
											public void onSuccess (Location[] result)
											{
												...
												for (Location s : result){
													locationHealthFacilityComboBox.addItem (s.getLocationName (), s.getLocationId ());
												}		
	...
	public void saveData()
	{
        ...
        newSetting.setDistrictId(locationDistrictComboBox.getValue(locationDistrictComboBox.getSelectedIndex()));
        newSetting.setHealthFacilityId(locationHealthFacilityComboBox.getValue(locationHealthFacilityComboBox.getSelectedIndex()));
        
        try{
        service.saveOtherMessageSetting (newSetting, new AsyncCallback<Boolean> ()
				{
	...
	private boolean otherValidate()
	{
		final StringBuilder errorMessage = new StringBuilder();
		valid = true;
		if(TBRClient.get(nameTextBox).equals("")) {

			errorMessage.append("Please enter a name.");
			valid = false;
		}
		
		if(TBRClient.get(cellNumberTextBox).equals("")) {
	...
	public void updateOtherData()
	{
		try
		{
			...
			service.updateOtherMessageSetting(newSetting, new AsyncCallback<Boolean>()
			{
				@Override
				public void onSuccess(Boolean result)
				{
	...
	public void deleteData()
	{
		try{
			...
			String id = TBRClient.get (alertRecipientListBox);
			try
			{
				service.findOtherMessageRecipientById (id,new AsyncCallback<OtherMessageSetting>(){
					...
					{
						service.deleteOtherMessageSetting (result,new AsyncCallback<Boolean>(){
		...
		try
		{
			service.findOtherMessageRecipientById (id,new AsyncCallback<OtherMessageSetting>(){
				...
				public void onSuccess(OtherMessageSetting result)
				{
					...
					locationDistrictComboBox.setSelectedIndex (TBRClient.getIndex (locationDistrictComboBox, result.getDistrictId()));
					...
					@Override
						public void onSuccess (Location[] result)
						{
							...
							locationHealthFacilityComboBox.setSelectedIndex (TBRClient.getIndex (locationHealthFacilityComboBox, otherSetting.getHealthFacilityId()));
							load (false);
						}
