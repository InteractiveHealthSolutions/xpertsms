...
public class UsersComposite extends Composite implements IForm, ClickHandler, ChangeHandler
{
	...
	private Label						lblTbReachUsers				= new Label("TB REACH Users");
	...
	public UsersComposite()
	{
		...
		leftFlexTable.getCellFormatter().setVerticalAlignment(1, 0, HasVerticalAlignment.ALIGN_MIDDLE);
	...
	private void refreshLists()
	{
		...
		for(String s : TBR.getSecretQuestions())
			secretQuestionComboBox.addItem(s);
		for (String s : TBR.getList(ListType.USER_STATUS))
			userStatusComboBox.addItem(s);
		for (String s : TBR.getList(ListType.USER_ROLE))
		{
	...
	public boolean validate()
	{
		...
		try
		{
			service.exists(tableName, "UserName='" + userNameTextBox.getText() + "'", new AsyncCallback<Boolean>()
			{
				@Override
				public void onSuccess(Boolean result)
				{
					if (result)
					{
						errorMessage.append("Username already exists. Please provide different user name.\n");
						valid = false;
					}
				}
				...
				{
					errorMessage.append(CustomMessage.getErrorMessage(ErrorType.DATA_ACCESS_ERROR) + "\n");
				}
			});
		}
		catch (Exception e)
	...
	public void saveData()
	{
		if (validate())
		{
			Users user = new Users(TBRClient.get(userNameTextBox).toUpperCase(), TBRClient.get(userNameTextBox).toUpperCase(), TBRClient.get(
					userRoleComboBox).toUpperCase(), TBRClient.get(userStatusComboBox).toUpperCase(), passwordTextBox.getText(), TBRClient.get(
					secretQuestionComboBox).toUpperCase(), secretAnswerPasswordTextBox.getText());
			try
			{
	...
	public void updateData()
	{
		if (validate())
		{
			try
			{
				service.findUser(currentUser.getUserName(), new AsyncCallback<Users>()
				{
					@Override
					public void onSuccess(Users result)
					{
						try
						{
							result.setRole(TBRClient.get(userRoleComboBox).toUpperCase());
							result.setStatus(TBRClient.get(userStatusComboBox).toUpperCase());
							result.setPassword(TBRClient.get(passwordTextBox));
							result.setSecretQuestion(TBRClient.get(secretQuestionComboBox).toUpperCase());
							result.setSecretAnswer(TBRClient.get(secretAnswerPasswordTextBox));
							service.updateUser(result, new AsyncCallback<Boolean>()
							{
								@Override
								public void onSuccess(Boolean result)
								{
	...
	public void fillData()
	{
		try
		{
			service.findUser(TBRClient.get(usersListBox), new AsyncCallback<Users>()
			{
				@Override
				public void onSuccess(Users result)
				{
					...
					userRoleComboBox.setSelectedIndex(TBRClient.getIndex(userRoleComboBox, result.getRole()));
					userStatusComboBox.setSelectedIndex(TBRClient.getIndex(userStatusComboBox, result.getStatus()));
					secretQuestionComboBox.setSelectedIndex(TBRClient.getIndex(secretQuestionComboBox, result.getSecretQuestion()));
					secretAnswerPasswordTextBox.setText(result.getSecretAnswer());
					load(false);
				}
	...
	{
		load(true);
		String selectedValue = TBRClient.get(userRolesComboBox);
		try
		{
			service.getColumnData(tableName, "UserName", "role='" + selectedValue + "'", new AsyncCallback<String[]>()
			{
				@Override
