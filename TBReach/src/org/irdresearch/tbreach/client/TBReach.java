 ...
 */
public class TBReach implements EntryPoint, ClickHandler
{
	// RPC Service
	...
	{
		Window.setTitle(TBR.projectTitle);
		...
		// 2. If cookies exist, call login method providing user name and pass code from cookies
		// 3. If browsing session has not expired, login method opens the main menu otherwise asks for renewing session
	...
	/**
	 * Handle User Login. If user is already logged in, main menu will display otherwise session renewal window will appear
	 */
	private void login()
	{
		String userName;
	...
	{
		rootPanel.clear();
		rootPanel.add(new HTML("Application has been shutdown. It is now safe to close the Browser window."));
	}
	...
	public static void logout()
	{
		try
		{
			service.recordLogout(TBR.getCurrentUser(), new AsyncCallback<Void>()
			{
				@Override
	...
	{
		Cookies.setCookie("UserName", TBR.getCurrentUser());
		Cookies.setCookie("Pass", TBR.getPassCode());
		Cookies.setCookie("LoginTime", String.valueOf(new Date().getTime()));
		Cookies.setCookie("SessionLimit", String.valueOf(new Date().getTime() + TBR.sessionLimit));
	}
	...
	public void onClick(ClickEvent event)
	{
		...
		if (sender == loginButton)
		{
			...
			if (TBRClient.get(loginNameTextBox).equals("") || TBRClient.get(passwordTextBox).equals(""))
			{
			...
			try
			{
				service.authenticate(TBRClient.get(loginNameTextBox), TBRClient.get(passwordTextBox), new AsyncCallback<Boolean>()
				{
					@Override
					public void onSuccess(Boolean result)
					{
						if (result)
						{
							...
							TBR.setCurrentUser(TBRClient.get(loginNameTextBox));
							TBR.setPassCode(String.valueOf(TBRClient.getSimpleCode(TBRClient.get(passwordTextBox).substring(0, 3))));
							removeCookies();
							setCookies();
							try
							{
								service.recordLogin(TBR.getCurrentUser(), new AsyncCallback<Void>()
								{
									@Override
									public void onSuccess(Void result)
									{
		...
		else if (sender == forgotPasswordButton)
		{
			...
			else
			{
				try
				{
					service.authenticateUser(loginNameTextBox.getText(), new AsyncCallback<Boolean>()
					{
						@Override
						public void onSuccess(Boolean result)
						{
							if (result)
							{
								TBR.setCurrentUser(loginNameTextBox.getText());
								// Set Secret question
								try
								{
									service.getSecretQuestion(loginNameTextBox.getText(), new AsyncCallback<String>()
									{
										@Override
										public void onSuccess(String result)
										{
		...
		else if (sender == recoveryButton)
		{
			...
			else
			{
				try
				{
					service.verifySecretAnswer(loginNameTextBox.getText(), answerTextBox.getText(), new AsyncCallback<Boolean>()
					{
						@Override
						public void onSuccess(Boolean result)
						{
		...
		else if (sender == setPasswordButton)
		{
			...
			else
			{
				try
				{
					service.updatePassword(loginNameTextBox.getText(), newPasswordTextBox.getText(), new AsyncCallback<Boolean>()
					{
						@Override
						public void onSuccess(Boolean result)
						{
