...
public class LocationComposite extends Composite implements IForm, ClickHandler, ChangeHandler
{
	...
	public LocationComposite ()
	{
		...
		districtNamesListBox.setTitle ("This list box contains Ditricts names. Clicking anyone fills details in right panel and associated facilities below.");
		...
		top2FlexTable.getCellFormatter ().setHorizontalAlignment (0, 0, HasHorizontalAlignment.ALIGN_CENTER);
		flexTable.setWidget (6, 0, left2FlexTable);
		facilityNamesListBox.setTitle ("This list box contains Facility names. Clicking anyone fills details in right panel.");
			...
			service.findLocationsByType (locationType, new AsyncCallback<Location[]>()
			{
				public void onSuccess (Location[] result)
				{
					...
					for (Location s : result){
						districtNamesListBox.addItem (s.getLocationName (), s.getLocationId ());
					}	
					load (false);
				}
	...
	public void fillData ()
	{
		try
		{
				service.findDistrict (TBRClient.get (districtNamesListBox), new AsyncCallback<Location> ()
				{
					@Override
					public void onSuccess (Location result)
					{
						...
						else
						{
							...
							cityTextBox.setValue (currentLocation.getAddressSector ());
							stateTextBox.setValue (currentLocation.getAddressColony ());*/
				...
				service.findFacilitiesByDistrictId (TBRClient.get (districtNamesListBox), new AsyncCallback<Location[]> ()
				{
					@Override
					public void onSuccess (Location[] result)
					{
						...
						for (Location s : result){
							facilityNamesListBox.addItem (s.getLocationName (), s.getLocationId ());
						}		
	...
	public boolean validate ()
	{
		...
		if (TBRClient.get (locationIdTextBox).equals ("") || TBRClient.get (locationNameTextBox).equals (""))
		{
			errorMessage.append (CustomMessage.getErrorMessage (ErrorType.EMPTY_DATA_ERROR) + "\n");
			valid = false;
		}
		else if (TBRClient.get (locationIdTextBox).length() != 2) {
	...
	public boolean validate2 ()
	{
		...
		valid = true;
		/* Validate mandatory fields */
		if (TBRClient.get (facilityIdTextBox).equals ("") || TBRClient.get (facilityNameTextBox).equals (""))
		{
			errorMessage.append (CustomMessage.getErrorMessage (ErrorType.EMPTY_DATA_ERROR) + "\n");
			valid = false;
		}
        else if (TBRClient.get (facilityIdTextBox).length() != 2) {
				...
				service.saveLocation (currentLocation, new AsyncCallback<Boolean> ()
				{
					public void onSuccess (Boolean result)
					{
						...
						else if (result)
						{
							...
							clearUp ();
							districtNamesListBox.addItem (currentLocation.getLocationName (), currentLocation.getLocationId ());
						}
						else
				...
				service.deleteLocation (currentLocation, new AsyncCallback<Boolean> ()
				{
					public void onSuccess (Boolean result)
					{
						if (result)
						{
							...
							clearUp ();
							districtNamesListBox.removeItem (TBRClient.getIndex (districtNamesListBox, currentLocation.getLocationId ()));
						}
						else
	...
	public void onClick (ClickEvent event)
	{
		...
		if (sender == create2Button)
		{
			...
			districtTextBox.setValue(districtNamesListBox.getItemText(districtNamesListBox.getSelectedIndex()));
			load (false);
		}
				...
				service.deleteLocation (currentLocation, new AsyncCallback<Boolean> ()
				{
					public void onSuccess (Boolean result)
					{
						if (result)
						{
							Window.alert (CustomMessage.getInfoMessage (InfoType.DELETED));
							clearUp ();
							districtNamesListBox.removeItem (TBRClient.getIndex (facilityNamesListBox, currentLocation.getLocationId ()));
						}
						else
							Window.alert (CustomMessage.getErrorMessage (ErrorType.DELETE_ERROR));
						load (false);
	...
	private void save2Data()
	{
		if (validate2 ())
		{
			try
			{
				...
				currentLocation.setCityId(TBRClient.get(districtNamesListBox));
				...
				service.saveLocation (currentLocation, new AsyncCallback<Boolean> ()
				{
					public void onSuccess (Boolean result)
					{
						...
						else if (result)
						{
							Window.alert (CustomMessage.getInfoMessage (InfoType.INSERTED));
							clearUp ();
							facilityNamesListBox.addItem (currentLocation.getLocationName (), currentLocation.getLocationId ());
						}
						else
							Window.alert (CustomMessage.getErrorMessage (ErrorType.INSERT_ERROR));
						load (false);
	...
	public void fillFacilityData (){
	  try
		{
		  districtTextBox.setValue(districtNamesListBox.getItemText(districtNamesListBox.getSelectedIndex()));
				service.findFacility (TBRClient.get (facilityNamesListBox), TBRClient.get (districtNamesListBox), new AsyncCallback<Location> ()
				{
					@Override
					public void onSuccess (Location result)
					{
						...
						{
							currentLocation = result;
							facilityIdTextBox.setValue (currentLocation.getLocationId ());
							facilityNameTextBox.setValue (currentLocation.getLocationName ());
							address1TextBox.setValue (currentLocation.getAddressHouse ());
							address2TextBox.setValue (currentLocation.getAddressStreet ());
