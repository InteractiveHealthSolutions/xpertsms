...
*/
	...
	{
		return (Location) HibernateUtil.util.findObject("from Location where LocationID='" + locationID + "' and locationType='DISTRICT'");
	}
	
	
	public Location findFacility (String facilityId, String districtId)
	{
		return (Location) HibernateUtil.util.findObject ("from Location where locationType='HEALTH FACILITY' and CityID='"+districtId+"' and LocationID='" + facilityId + "'");
	...
	public Boolean updateGeneXpertResults(GeneXpertResults geneXpertResults, Boolean isTBPositive) throws Exception
	{
		Boolean result = HibernateUtil.util.update(geneXpertResults);
			...
			SMSUtil.util.sendAlertsOnAutoGXPResults(geneXpertResults);//, isTBPositive, gpIncentive, chwIncentive);
	...
	{
		Object[] list = HibernateUtil.util.findObjects ("from Location where locationType='HEALTH FACILITY' and CityID='"+districtId+"'");
		Location[] locations = new Location[list.length];
		for (int i = 0; i < list.length; i++)
			locations[i] = (Location) list[i];
		return locations;
	...
	{
		Object[] list = HibernateUtil.util.findObjects ("from OtherMessageSetting");
	...
	{
		return (OtherMessageSetting) HibernateUtil.util.findObject ("from OtherMessageSetting where id = '"+id+"'");
	}
